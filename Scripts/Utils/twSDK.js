 ...
 */
    ...
    window.twSDK = {
        ...
        init: async function (scriptConfig) {
		  ...
		  // =========================================
  // Neue Module als Eigenschaften des twSDK-Objekts
  // =========================================
  
  // twSDK.Utils: Allgemeine Hilfsfunktionen
  Utils: {
    findRowByLabel: function($container, labelText) {
      return $container.find("tr").filter(function () {
        var cellText = jQuery(this).find("td, th").first().text().trim();
        return cellText === labelText;
      }).first();
    },
    parseResourceValue: function(valueStr) {
      var clean = valueStr.replace(/[.,\s]/g, "");
      return parseInt(clean, 10);
    },
    getLastCoordFromString: function(text) {
      if (!text) return "";
  ...
  // twSDK.DateUtils: Datum- und Zeitoperationen
  DateUtils: {
  ...
  // twSDK.ReportParser: Funktionen zum Auslesen von Berichten
  ReportParser: {
    ...
    parseReportType: function($htmlDoc) {
    ...
    // parseBuildingData: Parst das hidden-Feld der Gebäudedaten
    parseBuildingData: function(rawData) {
      var levels = {
        timberCamp: 0,
        clayPit: 0,
        ironMine: 0,
    ...
    extractUnitsFromTable: function($htmlDoc, tableSelector) {
      var units = {};
    ...
    extractLossesFromTable: function($htmlDoc, tableSelector) {
      var losses = {};
      this.unitList.forEach(function(unit) {
        losses[unit] = 0;
      });
    ...
    parseUnitsData: function($htmlDoc) {
      return {
        units: {
          attacker: this.extractUnitsFromTable($htmlDoc, "#attack_info_att_units"),
          defender: this.extractUnitsFromTable($htmlDoc, "#attack_info_def_units")
        },
        losses: {
          attacker: this.extractLossesFromTable($htmlDoc, "#attack_info_att_units"),
          defender: this.extractLossesFromTable($htmlDoc, "#attack_info_def_units")
        }
      };
    },
    
    // Extrahiert Ressourcen (sowohl "Resources scouted" als auch "Possible resources")
    parseResources: function($htmlDoc) {
    ...
    parseBuildings: function($htmlDoc) {
      var buildingList = [
      ...
      var parsedBuildings = {};
      if (buildingDataRaw) {
        parsedBuildings = this.parseBuildingData(buildingDataRaw);
      }
    ...
    parseAdditionalInfo: function($htmlDoc) {
      return {
        buffs: (function() {
    ...
    parseFullReport: function($htmlDoc) {
      var generalInfo = this.parseGeneralInfo($htmlDoc);
      var unitsData = this.parseUnitsData($htmlDoc);
      var resources = this.parseResources($htmlDoc);
      var buildings = this.parseBuildings($htmlDoc);
      var additionalInfo = this.parseAdditionalInfo($htmlDoc);
    ...
    // Allgemeine Information aus dem Bericht (allgemeine Felder)
    parseGeneralInfo: function($htmlDoc) {
      return {
        reportType: this.parseReportType($htmlDoc) || { type: null, dotIcon: null, lootStatus: null },
        battleTime: (function() {
          var $row = twSDK.Utils.findRowByLabel($htmlDoc, "Battle time");
          return $row.length ? $row.find("td").eq(1).text().trim() : null;
        })(),
        attacker: (function() {
          var $row = twSDK.Utils.findRowByLabel($htmlDoc, "Origin:");
          var coords = $row.length ? twSDK.Utils.getLastCoordFromString($row.find("td").eq(1).text().trim()) : null;
          var $name = $htmlDoc.find("#attack_info_att a[href*='screen=info_player']");
          return {
            name: $name.length ? twSDK.cleanString($name.first().text().trim()) : null,
            coords: coords
          };
        })(),
        defender: (function() {
          var $row = twSDK.Utils.findRowByLabel($htmlDoc, "Destination:");
          var coords = $row.length ? twSDK.Utils.getLastCoordFromString($row.find("td").eq(1).text().trim()) : null;
          var $name = $htmlDoc.find("#attack_info_def a[href*='screen=info_player']");
          return {
            name: $name.length ? twSDK.cleanString($name.first().text().trim()) : null,
            coords: coords
          };
